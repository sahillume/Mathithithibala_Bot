...
async function writeExifImg(img, metadata) {
  ...
  const json = {
    ...
    'sticker-pack-name': packname || 'Mathithibala Bot',
    emojis: ['🤖'],
  };
...
/**
 * Write EXIF metadata to video sticker
 */
async function writeExifVid(videoBuffer, metadata) {
  ...
  if (videoBuffer.length > MAX_FILE_SIZE) {
    throw new Error(`File too large: ${(videoBuffer.length / 1024 / 1024).toFixed(2)}MB`);
  }
